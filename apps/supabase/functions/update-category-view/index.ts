...
interface UpdateCategoryViewRequest {
  user_id: string;
  id: string;
  name: string;
  category_ids: string[];
...
serve(async (req) => {
  ...
  try {
    ...
    const { user_id, id, name, category_ids, category_group_ids }:
      UpdateCategoryViewRequest = requestBody;

    // Prepare parameters
    const rpcParams = {
      p_user_id: user_id,
      p_category_view_id: id,
      p_name: name,
